...
macro_rules! implement_signals_with_pipe {
    ($pipe:path) => {
        ...
        use std::io::{Error as MioIoError, Error as SignalError, ErrorKind};
        ...
        use libc::c_int;
        type Error = MioIoError;

        fn map_signal_error(error: SignalError) -> Error {
            if let Some(errno) = error.raw_os_error() {
                return MioIoError::from_raw_os_error(errno);
            }
            ...
            MioIoError::new(kind, "signal-hook-mio error")
        }
